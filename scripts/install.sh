...
install_systemd_units() {
  ...
  # Size the worker's memory ceiling to THIS host instead of shipping a fixed
  # guess. The unit's default MemoryMax=2G is a cgroup hard limit: when the
  # `claude` CLI (a Node process, whose footprint spikes on a real run) crosses
  # it, the kernel OOM-kills inside the cgroup and the service dies. Idle poll
  # ticks are cheap, so the symptom is "the worker runs for a while, then turns
  # itself off" - every time it actually tries to do work. A drop-in keeps the
  # shipped unit untouched and is re-applied cleanly on every install.
  local mem_total_kb mem_limit_mb tasks_max
  mem_total_kb="$(sed -n 's/^MemTotal:[[:space:]]*\([0-9]*\).*/\1/p' /proc/meminfo 2>/dev/null || echo 0)"
  if [[ "${mem_total_kb:-0}" -gt 0 ]]; then
    local host_mb hard_cap
    host_mb=$(( mem_total_kb / 1024 ))
    mem_limit_mb=$(( host_mb * 60 / 100 ))               # 60% of RAM
    [[ "$mem_limit_mb" -lt 1536 ]] && mem_limit_mb=1536  # a real claude run needs room
    [[ "$mem_limit_mb" -gt 8192 ]] && mem_limit_mb=8192  # no need to hand it the whole box
    # On a small host the floor above can exceed physical RAM, which would put
    # the cgroup limit out of reach and hand every overrun to the kernel OOM
    # killer (taking the rest of the stack with it). Never exceed 85% of RAM.
    hard_cap=$(( host_mb * 85 / 100 ))
    [[ "$hard_cap" -gt 0 && "$mem_limit_mb" -gt "$hard_cap" ]] && mem_limit_mb="$hard_cap"
  else
    mem_limit_mb=3072
  fi
  tasks_max=512
  mkdir -p /etc/systemd/system/rabit-claude-worker.service.d
  cat > /etc/systemd/system/rabit-claude-worker.service.d/10-resources.conf <<EOF
# Generated by scripts/install.sh - sized to this host's RAM. Do not edit;
...
TasksMax=${tasks_max}
EOF
  log_ok "Worker memory ceiling set to ${mem_limit_mb}M (host RAM: $(( ${mem_total_kb:-0} / 1024 ))M)"
